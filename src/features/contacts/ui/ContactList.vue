...
watch(unresolvedRoomSet, (set) => {
  ...
  if (nameRetryCount >= MAX_NAME_RETRIES) {
    // All retries exhausted — give up on these rooms, show fallback name instead of skeleton
    console.warn(
...
// If user profiles arrive late (e.g. from another sync path), remove gave-up flag so real name shows
